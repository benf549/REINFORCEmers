...
## =====  CHANGE HERE  ======== 
header = 'supervised' # or reinforce/ supervised
...
template_folder = f'/nfs/polizzi/cyao/RL/CS184/Project/finals/input_noxaa/0_template/reinforce/' # output
output_folder = f'/nfs/polizzi/cyao/RL/CS184/Project/finals/1_rmsd_rotafreq/{header}/' # output
...
def analyze_proteins(target_path, mobile_path, output_path, template_path, rota_freq_dict,target_file):
    ...
    basename = mobile_path.split('/')[-1]
    combined_df['basename'] = [basename]*len(combined_df)
    combined_df['model_type'] = [header]*len(combined_df)
...
def process_proteins(target_file):
    ...
    mobile_file = header + '_' + target_file.split('_')[-1] # e.g. supervised_00070.pdb
    mobile_path = os.path.join(mobile_folder, mobile_file)
    print(f' ==== mobile_path = {mobile_path} ===')
    output_path = os.path.join(output_folder, mobile_file.replace('.pdb', '.pkl'))
    template_path = os.path.join(template_folder, mobile_file.replace('.pdb', '.pkl'))
...
def main():
    ...
    target_files = [file.name for file in Path(target_folder).glob('*.pdb')] # now target_file = 'ground_truth_00038.pdb'
